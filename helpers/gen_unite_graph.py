...
on: gpt-oss, gemma4, nemotron-cascade-2, qwen3.5, qwen3.6.
...
def get_csv(filepath: str) -> pd.DataFrame:
    """
    Read a single CSV and explicitly attach kernel_version and campain name.
    """
    try:
...
def plot_chart(
    ...
    """Create a seaborn plot and save it to out_dir/<out_prefix>.png."""
...
# --------------------------------------------------------------------
# 3.  Read the JSON config files
# --------------------------------------------------------------------
def read_config(config_root: str) -> Dict[str, List[dict]]:
    """Walk config_root, load *.json, return { application_name: [plot_dict, …] }"""
...
def generate_html(
    ...
    for app_name, types in structured_plots.items():
...
def main() -> None:
    ...
    for app in df["app"].dropna().unique():
        ...
        exec_types = df_app["execution_type"].dropna().unique()
        if args.debug:
            print(f"Processing app: {app} ({len(exec_types)} execution types)")

        for etype in exec_types:
            df_et = df_app[df_app["execution_type"] == etype]
            ...
            for plot_def in plot_defs:
                ...
                if not x or not y or x not in df_et.columns or y not in df_et.columns:
                    continue
                ...
                out_path = plot_chart(params, df_et, out_dir,
                                      prefix, global_palette, args.debug)
